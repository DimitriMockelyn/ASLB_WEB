...
export default React.createClass({
    ...
    loadAllActivites() {
        homeServices.loadActivites().then(res => {this.setState({ptn: res})});
    },
    openPopin(ptn) {
    ...
    renderActionsEdit() {
        if (!this.props.hideButtons) {
            return <div>
                <Button type='button' label='admin.addActivite' handleOnClick={() => {this.openPopin()}}/>
                <Button type='button' label='button.voirPlus' handleOnClick={() => {this.setState({limit: this.state.limit+3})}}/>
            </div>
        }
    },
    delete(data) {
        adminServices.deleteActivite({id: data._id}).then(this.loadAllActivites);
    },
    /** @inheritDoc */
    renderContent() {
        return (
        <Panel  title='Activites' actions={this.renderActionsEdit}>
