...
export default React.createClass({
    ...
    loadAll() {
        agendaServices.loadTypeEvenements().then(res => {this.setState({ptn: res})});
    },
    openPopin(ptn) {
    ...
    renderActionsEdit() {
        if (!this.props.hideButtons) {
            return <div>
                <Button type='button' label='admin.addType' handleOnClick={() => {this.openPopin()}}/>
            </div>
        }
    },
    delete(data) {
        adminServices.deleteTypeEvenement({id: data._id}).then(this.loadAll);
    },
    /** @inheritDoc */
    renderContent() {
        return (
        <Panel  title="Types d'activité" actions={this.renderActionsEdit}>
            <div data-focus='news-list'>
                {this.state.ptn && this.state.ptn.length > 0 && this.state.ptn.map((value, pos) => {
                        return <DetailPanel value={value} editAction={this.openPopin} deleteAction={this.delete}/>
                })}
            </div>
