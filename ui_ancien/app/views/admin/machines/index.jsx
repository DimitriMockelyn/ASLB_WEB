...
export default React.createClass({
    ...
    loadAllMachines() {
        homeServices.loadMachines().then(res => {this.setState({ptn: res})});
    },
    openPopin(ptn) {
    ...
    renderActionsEdit() {
        if (!this.props.hideButtons) {
            return <div>
                <Button type='button' label='admin.addMachine' handleOnClick={() => {this.openPopin()}}/>
                <Button type='button' label='button.voirPlus' handleOnClick={() => {this.setState({limit: this.state.limit+3})}}/>
            </div>
        }
    },
    delete(data) {
        adminServices.deleteMachine({id: data._id}).then(this.loadAllMachines);
    },
    /** @inheritDoc */
    renderContent() {
        return (
        <Panel  title='Machines' actions={this.renderActionsEdit}>
