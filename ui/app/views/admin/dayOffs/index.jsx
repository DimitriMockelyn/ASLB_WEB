...
export default React.createClass({
    ...
    loadAllDayOffs() {
        homeServices.loadDayOffs().then(res => {this.setState({ptn: res})});
    },
    openPopin(ptn) {
    ...
    renderActionsEdit() {
        if (!this.props.hideButtons) {
            return <div>
                <Button type='button' label='admin.addDayOff' handleOnClick={() => {this.openPopin()}}/>
                <Button type='button' label='button.voirPlus' handleOnClick={() => {this.setState({limit: this.state.limit+3})}}/>
            </div>
        }
    },
    delete(data) {
        adminServices.deleteDayOff({id: data._id}).then(this.loadAllDayOffs);
    },
    /** @inheritDoc */
    renderContent() {
        return (
        <Panel  title='dayOff.panelTitle' actions={this.renderActionsEdit}>
